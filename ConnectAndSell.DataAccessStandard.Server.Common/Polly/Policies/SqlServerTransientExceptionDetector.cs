        ...
        public static bool ShouldRetryOn(SqlException ex)
        {
            foreach (SqlError err in ex.Errors)
            {
                switch (err.Number)
                {
                    ...
                    // the remote host.)
                    case 229:
                    // Resolving PBI 6554835 to retry error so that Permission can be captured and granted if in test mode
                    /* The ??? permission was denied on the object ‘???’, database '???', schema 'dbo'.   (Microsoft SQL Server, Error: 229) */
                        return true;
                    case 233:
                        return true;
                    // SQL Error Code: 121
